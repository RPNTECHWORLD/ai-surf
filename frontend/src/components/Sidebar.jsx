...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { label: 'Students', path: '/students', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v2a4 4 0 0 1-4 4H5a4 4 0 0 1-4-4v-2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg> },
    ...
    { label: 'Register School', path: '/register', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg> },
  ];
    ...
    <aside className="db-sidebar">
      ...
      <nav className="db-nav">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.label} className={`db-nav-item${isActive ? ' db-nav-active' : ''}`} onClick={() => navigate(item.path)}>
              {item.icon}
              {item.label}
            </button>
          );
